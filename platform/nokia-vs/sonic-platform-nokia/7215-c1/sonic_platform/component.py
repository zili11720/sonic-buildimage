...
class Component(ComponentBase):
    ...
    CHASSIS_COMPONENTS = [
        ...
        ["FPGA", "Manages RS232 console ports "],
    ]
    ...
    def _get_fpga_version(self, fpga_number):
        fpga_version = self._read_sysfs_file(FPGA_DIR + "version")
        return fpga_version
    ...
    def get_firmware_version(self):
        ...
        if self.index == 2:
            return self._get_fpga_version(self.index)
    ...
    def install_firmware(self, image_path):
        ...
        """
        image_name = ntpath.basename(image_path)
        print(" Nokia - install firmware {}".format(image_name))
        ...
        if self.name == "System-CPLD":
            ...
            try:
                if not os.path.isdir(mount_point):
                    subprocess.check_call(FPGA_UPDATE_COMMAND1, stderr=subprocess.STDOUT)
                if not os.path.ismount(mount_point):
                    subprocess.check_call(FPGA_UPDATE_COMMAND2, stderr=subprocess.STDOUT)
                subprocess.check_call(FPGA_UPDATE_COMMAND3, stderr=subprocess.STDOUT)
                subprocess.check_call(FPGA_UPDATE_COMMAND4, stderr=subprocess.STDOUT)
                subprocess.check_call(FPGA_UPDATE_COMMAND5, stderr=subprocess.STDOUT)
                subprocess.check_call(FPGA_UPDATE_COMMAND6, stderr=subprocess.STDOUT)
                success_flag = True
            except subprocess.CalledProcessError as e:
                print("ERROR: Failed to upgrade FPGA: rc={}".format(e.returncode))

            if success_flag:
                print("INFO: Power cycle is required to finish FPGA installation")
