...
#define SFP_PRESENCE_REG_SFP1         0x2
#define SFP_PRESENCE_REG_SFP2         0x3

#define SFP_LOS_REG_SFP1              0x2
#define SFP_LOS_REG_SFP2              0x3

#define SFP_TX_DISABLE_REG_SFP1       0x2
#define SFP_TX_DISABLE_REG_SFP2       0x3
...
// sfp bit offset
#define SFP_LED_LINK_SFP1             7
#define SFP_LED_ACT_SFP1              6
#define SFP_LED_LINK_SFP2             5
#define SFP_LED_ACT_SFP2              4
...
#define SFP_TX_FAULT_STATUS_SFP1   0x2
#define SFP_TX_FAULT_STATUS_SFP2   0x3
...
//========================================= CPLD DRIVER GPIO FUNCTIONS =========================================

// SPDX-License-Identifier: GPL-2.0
//
// gpio-cpld-sfp.c - I2C CPLD GPIO provider for two SFP ports (SFP1, SFP2)
// Exposes 8 GPIO lines total: (presence, los, tx_disable, tx_fault) x 2 ports
//
// Based on the Nokia-7215-IXS-A1 CPLD register map
//
// GPIO offsets we export (0..11):
//   0: SFP1_PRESENT   (input) -> reg 0x03 bit 2
//   1: SFP1_LOS       (input) -> reg 0x04 bit 2
//   2: SFP1_TX_DISABLE(output)-> reg 0x05 bit 2
//   3: SFP1_TX_FAULT  (input) -> reg 0x09 bit 2
//   4: SFP2_PRESENT   (input) -> reg 0x03 bit 3
//   5: SFP2_LOS       (input) -> reg 0x04 bit 3
//   6: SFP2_TX_DISABLE(output)-> reg 0x05 bit 3
//   7: SFP2_TX_FAULT  (input) -> reg 0x09 bit 3
//   8: SFP1_LINK     (input) -> reg 0x0F bit 7
//   9: SFP1_ACT      (input) -> reg 0x0F bit 6
//  10: SFP2_LINK     (input) -> reg 0x0F bit 5
//  11: SFP2_ACT      (input) -> reg 0x0F bit 4

/* ====== GPIO line mapping (offsets) ====== */
enum {
    NOKIA_CPLD_GPIO_SFP1_PRESENT = 0, /* in  */
    NOKIA_CPLD_GPIO_SFP1_LOS,         /* in  */
    NOKIA_CPLD_GPIO_SFP1_TXDIS,       /* out */
    NOKIA_CPLD_GPIO_SFP1_TXFLT,       /* in  */
    NOKIA_CPLD_GPIO_SFP2_PRESENT,     /* in  */
    NOKIA_CPLD_GPIO_SFP2_LOS,         /* in  */
    NOKIA_CPLD_GPIO_SFP2_TXDIS,       /* out */
    NOKIA_CPLD_GPIO_SFP2_TXFLT,       /* in  */

    NOKIA_CPLD_GPIO_SFP1_LINK,       /* out  */
    NOKIA_CPLD_GPIO_SFP1_ACT,        /* out  */
    NOKIA_CPLD_GPIO_SFP2_LINK,       /* out  */
    NOKIA_CPLD_GPIO_SFP2_ACT,        /* out  */
    NOKIA_CPLD_GPIO_LINES            /* == 12 */
};
...
struct cpld_sfp {
    ...
    struct gpio_chip   gc;
    struct cpld_data *data_ptr;
    /* If your CPLD has explicit DIR registers, add fields here. */
};

static const struct cpld_line cpld_lines[NOKIA_CPLD_GPIO_LINES] = {
    /* SFP1 */
    [NOKIA_CPLD_GPIO_SFP1_PRESENT] = { SFP_PRESENCE_REG,        SFP_PRESENCE_REG_SFP1,      false },
    [NOKIA_CPLD_GPIO_SFP1_LOS]     = { SFP_LOS_REG,             SFP_LOS_REG_SFP1,           false },
    [NOKIA_CPLD_GPIO_SFP1_TXDIS]   = { SFP_TX_DISABLE_REG,      SFP_TX_DISABLE_REG_SFP1,     true },
    [NOKIA_CPLD_GPIO_SFP1_TXFLT]   = { SFP_TX_FAULT_STATUS_REG, SFP_TX_FAULT_STATUS_SFP1,   false },
    /* SFP2 */
    [NOKIA_CPLD_GPIO_SFP2_PRESENT] = { SFP_PRESENCE_REG,        SFP_PRESENCE_REG_SFP2,      false },
    [NOKIA_CPLD_GPIO_SFP2_LOS]     = { SFP_LOS_REG,             SFP_LOS_REG_SFP2,           false },
    [NOKIA_CPLD_GPIO_SFP2_TXDIS]   = { SFP_TX_DISABLE_REG,      SFP_TX_DISABLE_REG_SFP2,     true },
    [NOKIA_CPLD_GPIO_SFP2_TXFLT]   = { SFP_TX_FAULT_STATUS_REG, SFP_TX_FAULT_STATUS_SFP2,   false },

    [NOKIA_CPLD_GPIO_SFP1_LINK]    = { SFP_LED_TEST_REG,        SFP_LED_LINK_SFP1,           true },
    [NOKIA_CPLD_GPIO_SFP1_ACT]     = { SFP_LED_TEST_REG,        SFP_LED_ACT_SFP1,            true },
    [NOKIA_CPLD_GPIO_SFP2_LINK]    = { SFP_LED_TEST_REG,        SFP_LED_LINK_SFP2,           true },
    [NOKIA_CPLD_GPIO_SFP2_ACT]     = { SFP_LED_TEST_REG,        SFP_LED_ACT_SFP2,            true },
};

/* ---- gpiolib ops ---- */

static int cpld_get_direction(struct gpio_chip *gc, unsigned int offset)
{
    /* 1 = input, 0 = output */
    if (offset >= NOKIA_CPLD_GPIO_LINES)
        return -EINVAL;

    return cpld_lines[offset].is_output ? GPIO_LINE_DIRECTION_OUT
                          : GPIO_LINE_DIRECTION_IN;
}

static int cpld_direction_input(struct gpio_chip *gc, unsigned int offset)
{
    if (offset >= NOKIA_CPLD_GPIO_LINES)
        return -EINVAL;
    /* If CPLD has DIR registers, set input here; otherwise no-op. */
    if (cpld_lines[offset].is_output)
        return -EINVAL; /* this line is output-only by design */
    return 0;
}

static int cpld_direction_output(struct gpio_chip *gc,
                   unsigned int offset, int value)
{
    struct cpld_sfp *c = gpiochip_get_data(gc);
    unsigned int mask, v;
    ...
    mask = BIT(cpld_lines[offset].bit);
    v    = value ? mask : 0;

    /* If HW has DIR register, set output dir first. Then set level. */
    return regmap_update_bits(c->regmap, cpld_lines[offset].reg, mask, v);
}

static int cpld_get(struct gpio_chip *gc, unsigned int offset)
{
    struct cpld_sfp *c = gpiochip_get_data(gc);
    unsigned int val;
    int ret;
    ...
    if (offset >= NOKIA_CPLD_GPIO_LINES)
        return -EINVAL;
    mutex_lock(&c->data_ptr->update_lock);
    ret = regmap_read(c->regmap, cpld_lines[offset].reg, &val);
    mutex_unlock(&c->data_ptr->update_lock);

    if (ret)
        return ret;

    return !!(val & BIT(cpld_lines[offset].bit));
}

static void cpld_set(struct gpio_chip *gc, unsigned int offset, int value)
{
    struct cpld_sfp *c = gpiochip_get_data(gc);
    unsigned int mask, v;
    ...
    v    = value ? mask : 0;
    mutex_lock(&c->data_ptr->update_lock);
    regmap_update_bits(c->regmap, cpld_lines[offset].reg, mask, v);
    mutex_unlock(&c->data_ptr->update_lock);
}

/* ---- I2C / regmap plumbing ---- */

static const struct regmap_config cpld_regmap_cfg = {
    .reg_bits = 8,
    .val_bits = 8,
    .max_register = 0xFF,
...
static int cpld_gpio_init(struct cpld_data *data)
{
    struct cpld_sfp *c;
    u32 ngpios = NOKIA_CPLD_GPIO_LINES;
    int ret;
    ...
    c->regmap = devm_regmap_init_i2c(client, &cpld_regmap_cfg);
    if (IS_ERR(c->regmap))
        return dev_err_probe(&client->dev, PTR_ERR(c->regmap),
                     "regmap init failed\n");

    /* Optional DT override, but must be >= 12 if present */
    of_property_read_u32(client->dev.of_node, "ngpios", &ngpios);
    if (ngpios < NOKIA_CPLD_GPIO_LINES)
        ngpios = NOKIA_CPLD_GPIO_LINES;

    /* gpio_chip init */

    c->gc.label            = dev_name(&client->dev);
    c->gc.parent           = &client->dev;
    c->gc.fwnode           = dev_fwnode(&client->dev);
    c->gc.owner            = THIS_MODULE;
    c->gc.base             = -1;
    c->gc.ngpio            = ngpios;
    c->gc.can_sleep        = true;
    ...
    c->gc.get              = cpld_get;
    c->gc.set              = cpld_set;
    c->gc.get_direction    = cpld_get_direction;
    c->gc.direction_input  = cpld_direction_input;
    c->gc.direction_output = cpld_direction_output;

    ret = devm_gpiochip_add_data(&client->dev, &c->gc, c);
    if (ret)
        return dev_err_probe(&client->dev, ret, "gpiochip add failed\n");

    dev_info(&client->dev,
         "CPLD SFP GPIO ready: 8 lines @ I2C 0x%02x (pres 0x03, los 0x04, txdis 0x05, fault 0x09)\n",
         client->addr);
    return 0;
}

//========================================= CPLD DRIVER HWMON FUNCTIONS =========================================
...
static SENSOR_DEVICE_ATTR(sfp1_present, S_IRUGO, show_sfp_present, NULL, SFP_PRESENCE_REG_SFP1);
static SENSOR_DEVICE_ATTR(sfp2_present, S_IRUGO, show_sfp_present, NULL, SFP_PRESENCE_REG_SFP2);
static SENSOR_DEVICE_ATTR(sfp1_los, S_IRUGO, show_sfp_los, NULL, SFP_LOS_REG_SFP1);
static SENSOR_DEVICE_ATTR(sfp2_los, S_IRUGO, show_sfp_los, NULL, SFP_LOS_REG_SFP2);
static SENSOR_DEVICE_ATTR(sfp1_tx_disable, S_IRUGO | S_IWUSR, show_sfp_tx_disable, set_sfp_tx_disable, SFP_LOS_REG_SFP1);
static SENSOR_DEVICE_ATTR(sfp2_tx_disable, S_IRUGO | S_IWUSR, show_sfp_tx_disable, set_sfp_tx_disable, SFP_LOS_REG_SFP2);
...
static SENSOR_DEVICE_ATTR(sfp1_tx_fault, S_IRUGO, show_sfp_tx_fault, NULL, SFP_TX_FAULT_STATUS_SFP1);
static SENSOR_DEVICE_ATTR(sfp2_tx_fault, S_IRUGO, show_sfp_tx_fault, NULL, SFP_TX_FAULT_STATUS_SFP2);
...
static struct attribute *nokia_7215_ixs_a1_cpld_attributes[] = {
    ...
    &sensor_dev_attr_sfp1_los.dev_attr.attr,
