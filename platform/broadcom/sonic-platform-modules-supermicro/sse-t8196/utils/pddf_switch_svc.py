...
def stop_platform_svc():
    status, output = subprocess.getstatusoutput("systemctl stop sse-t8196-platform-init.service")
    if status:
        print "Stop sse-t8196-platform-init.service failed %d"%status
        return False
    status, output = subprocess.getstatusoutput("systemctl disable sse-t8196-platform-init.service")
    if status:
        print "Disable sse-t8196-platform-init.service failed %d"%status
        return False

    status, output = subprocess.getstatusoutput("/usr/local/bin/sse-t8196_util.py clean")
    if status:
        print "sse-t8196_util.py clean command failed %d"%status
        return False
...
def start_platform_svc():
    status, output = subprocess.getstatusoutput("/usr/local/bin/sse-t8196_util.py install")
    if status:
        print "sse-t8196_util.py install command failed %d"%status
        return False

    status, output = subprocess.getstatusoutput("systemctl enable sse-t8196-platform-init.service")
    if status:
        print "Enable sse-t8196-platform-init.service failed %d"%status
        return False
    status, output = subprocess.getstatusoutput("systemctl start sse-t8196-platform-init.service")
    if status:
        print "Start sse-t8196-platform-init.service failed %d"%status
        return False

    return True

def start_platform_pddf():
    # Enable PDDF 2.0 object class for sse-t8196
...
def stop_platform_pddf():
    # Disable PDDF 2.0 object class for sse-t8196
