...
def portio_reg_read(resource, offset):
    """ Port IO register read """
    file_des = os.open(resource, os.O_RDONLY)
    if file_des < 0:
        print('file open failed %s"%resource')
        return
    if os.lseek(file_des, offset, os.SEEK_SET) != offset:
        print('lseek failed on %s'%resource)
        return
    buf = os.read(file_des, 1)
    reg_val1 = ord(buf)
    print('reg value %x'%reg_val1)
    os.close(file_des)

def portio_reg_write(resource, offset, val):
    """ Port IO register write """
    file_des = os.open(resource, os.O_RDWR)
    if file_des < 0:
        print('file open failed %s"%resource')
        return
    if os.lseek(file_des, offset, os.SEEK_SET) != offset:
        print('lseek failed on %s'%resource)
        return
    ret = os.write(file_des, struct.pack('B', val))
    if ret != 1:
