...
# Platform init script for Dell S3248T
...
sys_devices() {
    case $1 in
        ...
        "delete_device") i2c_config "echo 0x50 > /sys/bus/i2c/devices/i2c-2/$1"
                         ...
                         ;;
        *)               echo "s3248t_platform: main_board_mux : invalid command !"
                         ;;
    esac
}
...
switch_board_sfp() {
    case $1 in
        ...
        "media_down")
                         ...
                         ;;
        *)               echo "s3248t_platform: switch_board_sfp: invalid command !"
                         ;;
    esac
}
...
echo "CPU CPLD: $((`cat /sys/devices/platform/dell-s3248t-cpld.0/cpu_cpld_mjr_ver`)).$((`cat /sys/devices/platform/dell-s3248t-cpld.0/cpu_cpld_mnr_ver`))" >> $FIRMWARE_VERSION_FILE
# Get SYS CPLD version
echo "SYS CPLD: $((`cat /sys/devices/platform/dell-s3248t-cpld.0/sys_cpld_mjr_ver`)).$((`cat /sys/devices/platform/dell-s3248t-cpld.0/sys_cpld_mnr_ver`))" >> $FIRMWARE_VERSION_FILE
...
get_reboot_cause() {
    ...
    if [[ ! -e $REBOOT_REASON_FILE ]]; then
        ...
        cat /sys/devices/platform/dell-s3248t-cpld.0/reboot_cause > $REBOOT_REASON_FILE
    fi
    echo "0x0" > /sys/devices/platform/dell-s3248t-cpld.0/reboot_cause
}
...
if [[ "$1" == "init" ]]; then
    ...
    modprobe pmbus
    modprobe emc2305
    modprobe dell_s3248t_platform

    sys_devices "new_device"
    get_reboot_cause
    switch_board_sfp "new_device"
    switch_board_sfp "media_down"
    echo 0xf0 > /sys/devices/platform/dell-s3248t-cpld.0/sfp_txdis
    echo 0xf3 > /sys/devices/platform/dell-s3248t-cpld.0/qsfp_rst
    echo 0x00 > /sys/devices/platform/dell-s3248t-cpld.0/qsfp_lpmode
    install_python_api_package
    platform_firmware_versions
elif [[ "$1" == "deinit" ]]; then
    switch_board_sfp "media_down"
    ...
    modprobe -r dell_s3248t_platform
    ...
    switch_board_sfp $1
     ...
     echo "s3248t_platform : Invalid option !"
fi
