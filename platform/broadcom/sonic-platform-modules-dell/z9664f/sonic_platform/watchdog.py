...
class Watchdog(WatchdogBase):
    ...
    def _get_reg_val(self):
        ...
        value = self._get_command_result("/usr/sbin/i2cget -y 600 0x38 0x07")
        if not value:
            return None
        else:
            return int(value, 16)
    ...
    def _set_reg_val(self, val):
        # 0x31 = CPLD I2C Base Address
        # 0x07 = Watchdog Function Register
        value = self._get_command_result("/usr/sbin/i2cset -y 600 0x38 0x07 %s"
                % (val))
        return value
    ...
    def get_remaining_time(self):
        ...
        Returns:
            ...
            Z9664F doesnot have hardware support to show remaining time.
