...
def stop_platform_svc():
    status, output = subprocess.getstatusoutput("systemctl stop platform-modules-z9664f.service")
    if status:
        print("Stop platform-modules-z9664f.service failed %d"%status)
        return False
    status, output = subprocess.getstatusoutput("systemctl disable platform-modules-z9664f.service")
    if status:
        print("Disable platform-modules-z9664f.service failed %d"%status)
        return False
...
def start_platform_svc():
    status, output = subprocess.getstatusoutput("systemctl enable platform-modules-z9664f.service")
    if status:
        print("Enable platform-modules-z9664f.service failed %d"%status)
        return False
    status, output = subprocess.getstatusoutput("systemctl start platform-modules-z9664f.service")
    if status:
        print("Start platform-modules-z9664f.service failed %d"%status)
        return False
