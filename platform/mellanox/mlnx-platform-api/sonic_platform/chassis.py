...
REVISION = "REV"
VPD_DATA_WAIT_TIMEOUT = 25  # Timeout in seconds for waiting for VPD data file
...
class Chassis(ChassisBase):
    ...
    def wait_for_vpd_data_creation(self, file_path, timeout):
        """
        ...
        # Timeout reached or error occurred, check if file exists now
        if os.access(file_path, os.R_OK):
            return True
    ...
    def _parse_vpd_data(self, filename):
        ...
        try:
            if not os.access(filename, os.R_OK):
                ...
                if not self.wait_for_vpd_data_creation(filename, VPD_DATA_WAIT_TIMEOUT):
