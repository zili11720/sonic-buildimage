...
#define WB_COMMON_DEBUG_VERBOSE(fmt, args...) do {                                        \
    if (g_common_debug_verbose) { \
        printk(KERN_ERR "[WB_COMMON][VER][func:%s line:%d]\r\n"fmt, __func__, __LINE__, ## args); \
    } \
} while (0)

#define WB_COMMON_DEBUG_ERROR(fmt, args...) do {                                        \
    if (g_common_debug_error) { \
        printk(KERN_ERR "[WB_COMMON][ERROR][func:%s line:%d]\r\n"fmt, __func__, __LINE__, ## args); \
    } \
} while (0)
...
int dfd_get_my_card_type(void)
{
    ...
    while (cnt--) {
        ...
        if (type < 0) {
            WB_COMMON_DEBUG_ERROR("__dfd_get_my_card_type fail cnt %d, ret %d.\n", cnt, type);
            msleep(PLATFORM_CARDTYPE_RETRY_TIMES);
            continue;
        }
        WB_COMMON_DEBUG_VERBOSE("success to get type 0x%x.\n", type);
        break;
    }
...
static int __init wb_common_init(void)
{
    int ret;

    WB_COMMON_DEBUG_VERBOSE("Enter.\n");
    ret = dfd_get_my_card_type();
    if (ret <= 0) {
        WB_COMMON_DEBUG_ERROR("dfd_get_my_card_type failed, ret %d.\n", ret);
        printk(KERN_ERR "Warning: Device type get failed, please check the TLV-EEPROM!\n");
        return -1;
    }

    WB_COMMON_DEBUG_VERBOSE("Leave success type 0x%x.\n", ret);
    return 0;
}

static void __exit wb_common_exit(void)
{
    WB_COMMON_DEBUG_VERBOSE("Exit.\n");
}

module_init(wb_common_init);
module_exit(wb_common_exit);
