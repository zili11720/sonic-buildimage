...
class FanUtil(FanBase):
    ...
    def get_num_fans(self):
        S3248T_MAX_FANTRAYS = 3
        return S3248T_MAX_FANTRAYS

    def get_presence(self, idx):
        sysfs_path = "/sys/devices/platform/dell-s3248t-cpld.0/fan" + self._fan_mapping[idx] + "_prs"
        return int(open(sysfs_path).read(), 16)

    def get_direction(self, idx):
        sysfs_path = "/sys/devices/platform/dell-s3248t-cpld.0/fan" + self._fan_mapping[idx] + "_dir"
