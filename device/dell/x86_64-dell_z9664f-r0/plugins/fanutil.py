...
class FanUtil(FanBase):
    ...
    def __init__(self, log_level=logging.DEBUG):
        ...
        self.num_fans = self.FANTRAY_NUM_ON_MAIN_BOARD \
            * self.NUM_FANS_PERTRAY

    def get_fan_status(self, fan_id):
        try:
            (ret_status, ipmi_cmd_ret) = \
                commands.getstatusoutput(self.IPMI_FAN_PRESENCE.format(fan_id))
            if ret_status == 0:
                return ipmi_cmd_ret.splitlines()[5].strip(' '
                        ).strip('[]')
        except Exception:
            logging.error('Failed to execute : %s'
                          % self.IPMI_FAN_PRESENCE.format(fan_id))

    def get_front_fan_speed(self, fan_id):
        try:
            (ret_status, ipmi_cmd_ret) = \
                commands.getstatusoutput(self.IPMI_FAN_FRONT_SPEED.format(fan_id))
            if ret_status == 0:
                rdata = ipmi_cmd_ret.splitlines()[3].split(':'
                        )[1].split(' ')[1]
                return rdata
        except Exception:
            logging.error('Failed to execute : %s'
                          % self.IPMI_FAN_FRONT_SPEED.format(fan_id))

    def get_rear_fan_speed(self, fan_id):
        try:
            (ret_status, ipmi_cmd_ret) = \
                commands.getstatusoutput(self.IPMI_FAN_REAR_SPEED.format(fan_id))
            if ret_status == 0:
                rdata = ipmi_cmd_ret.splitlines()[3].split(':'
                        )[1].split(' ')[1]
                return rdata
            ...
            logging.error('Failed to execute : %s'
                          % self.IPMI_FAN_REAR_SPEED.format(fan_id))
    ...
    def get_fan_direction_from_fru(self, fru_id, reg_name):
        output = None
        try:
            (status, ipmi_fru_list) = \
                commands.getstatusoutput(self.IPMI_FRU_DATA.format(fru_id))
            if status == 0:
    ...
    def get_presence(self, index):
        ...
        if index < self.FANTRAY_NUM_START_IDX or index \
            > self.FANTRAY_NUM_START_IDX + self.num_fans - 1:
    ...
    def get_direction(self, index):
        ...
        direction = self.get_fan_direction_from_fru(fru_id,
                'Board Extra')
